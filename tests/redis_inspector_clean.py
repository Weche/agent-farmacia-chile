#!/usr/bin/env python3
"""
Redis Session Inspector (Windows Compatible)
...
async def inspect_redis_sessions():
    ...
    try:
        ...
        if not session_keys:
            print("SUCCESS: No sessions found - Redis is clean!")
            return
            
    except Exception as e:
        print(f"ERROR: Error getting session keys: {e}")
        return
            ...
            if very_old:
                ...
                for session in very_old:
                    try:
                        ...
                        print(f"  SUCCESS: Cleaned {session['session_id'][:12]}...")
                    except Exception as e:
                        print(f"  ERROR: Error cleaning {session['session_id'][:12]}...: {e}")
            ...
            if invalid_sessions:
                ...
                for key in invalid_sessions:
                    try:
                        ...
                        print(f"  SUCCESS: Cleaned {key}")
                    except Exception as e:
                        print(f"  ERROR: Error cleaning {key}: {e}")
                print(f"Cleanup complete!")
            else:
            ...
            for convo_key in convo_keys:
                ...
                if convo_session_id not in session_ids:
                    try:
                        ...
                        cleaned += 1
                        print(f"  SUCCESS: Cleaned orphaned conversation for {convo_session_id[:12]}...")
                    except Exception as e:
                        print(f"  ERROR: Error cleaning conversation {convo_session_id[:12]}...: {e}")
